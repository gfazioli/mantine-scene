...
`Scene.Sparkles` scatters random twinkling dots across the scene. Each dot has a randomised size, duration and phase so the field "breathes" rather than pulsing in sync. Lower `density` (0..1) to thin the field responsively without changing `count`.

<Demo data={demos.sparklesConfigurator} />
